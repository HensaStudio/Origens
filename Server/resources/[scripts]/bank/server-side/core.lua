...
-- ADDTAXS
-----------------------------------------------------------------------------------------------------------------------------------------
exports("AddTaxs",function(Passport,source,Name,Valuation,Message,Silenced)
	...
	if not Silenced then
		TriggerClientEvent("Notify",source,"Banco Fleeca","Débito de <b>"..Currency..""..Dotted(Valuation).."</b> adicionado à sua conta por <b>"..Name.."</b>.","dollar",5000)
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- CHECKTAXS
...
exports("CheckTaxs",function(Passport)
	...
	if Passport and source and exports.oxmysql:single_async("SELECT * FROM taxs WHERE Passport = ? AND CURDATE() >= DATE_ADD(STR_TO_DATE(`Date`,'%d/%m/%Y'),INTERVAL 1 DAY) LIMIT 1",{ Passport }) then
		TriggerClientEvent("Notify",source,"Banco Fleeca","Você possui débitos bancários.","dollar",5000)
		return true
	end
