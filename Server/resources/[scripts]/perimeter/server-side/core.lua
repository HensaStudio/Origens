...
AddEventHandler("perimeter:New",function()
    ...
    if not Passport or not vRP.HasGroup(Passport,"Police") then return end
...
AddEventHandler("perimeter:Remove",function(Selected)
    local Source = source
    local Passport = vRP.Passport(Source)
    if not Perimeters[Selected] or not Passport or not vRP.HasGroup(Passport,"Police") then return end
