...
function Creative.Code(Code)
	...
	if Passport then
		if not Code or Code == "" then
			TriggerClientEvent("pause:Notify", source, "Aviso", "Código inválido.", "vermelho")
			return false
		end

		local CodeData = vRP.GetRedeemCode(Code)
		if not CodeData then
			TriggerClientEvent("pause:Notify", source, "Aviso", "Código não encontrado.", "vermelho")
			return false
		end

		local Success, Message = vRP.RedeemCode(Passport, Code)
		if Success then
			TriggerClientEvent("pause:Notify", source, "Sucesso", Message or "Código resgatado com sucesso.", "verde")
		else
			TriggerClientEvent("pause:Notify", source, "Aviso", Message or "Código inválido ou já utilizado.", "vermelho")
		end
